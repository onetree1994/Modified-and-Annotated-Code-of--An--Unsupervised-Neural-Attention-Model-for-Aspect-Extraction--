...
# 获取logger实例，如果参数为空则返回root logger。提供日志接口，供应用代码使用。logger最常用的操作有两类：配置和发送日志消息。
...
# 读取数据：词典、训练数据、测试数据、每个review最大长度；数据的形式是二维数组，每一行是一个review, 每行中的具体元素是数字，表示该词在词典中的次序。
